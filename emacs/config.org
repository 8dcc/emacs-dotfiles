...
Return a string representing the date for tomorrow. Used by [[*Org capture][Org capture]].

TODO: We could add some inputs to this function.
...
We will also highlight the current line in programming modes (and Dired).
...
Try to reuse the same window for some buffers (e.g. =*grep*= or =*vc*=). If it's not
