...
We set =xref-show-definitions-function= and =xref-show-xrefs-function= to
=#'consult-xref= so functions like =xref-find-definitions= and =xref-find-references=
don't create their own =*xref*= buffers, but instead show a [[*Consult][Consult]] popup. We also
set =xref-prompt-for-identifier= to =nil= so it doesn't ask us for the symbol we are
looking for every time; only when it has no choice.
...
TODO: Move to [[*Org mode][Org mode]]? Organize all =x8dcc/...= functions.
...
Alternative to =kill-buffer-and-window= that uses =evil-window-delete=. Bound to =SPC w C=.
...
TODO: The [[https://man.cx/a2ps][a2ps]] command might be worth checking for font sizes and other settings.
...
Prefer user labels when exporting to LaTeX or HTML.
...
*** COMMENT Highlight extra constants

Add some keywords to the buffer-local =c-constant-key= variable.

FIXME: This doesn't work as expected, even though the variable is being set to
the right value.
...
(defconst x8dcc/c-constant-regexp
  (concat (regexp-opt x8dcc/c-constant-list)
          "\\([^[:alnum:]_$]\\|$\\)")
  "Regular expression for overwritting `c-constant-key' in `c-mode-hook'.

Based on the original value of `c-constant-key' at the time of writing
this (cc-mode 5.33.1):
\"\\(\\(?:NULL\\|\\(?:fals\\|tru\\)e\\)\\)\\([^[:alnum:]_$]\\|$\\)\"")

(add-hook 'c-mode-hook
          (lambda ()
            (setq-local c-constant-key x8dcc/c-constant-regexp)))
#+end_src
