...
2. Disable =-Wunused-function= warnings. It would be better to disable them just
   in header files, though.
...
A Language Server Protocol client for Emacs. For more information, see the [[https://joaotavora.github.io/eglot/][Eglot
manual]].

We don't use =use-package= since it's built into Emacs, and we don't want to
re-download more dependencies.

NOTE: This mode isn't hooked anywhere, so you will have to run =M-x eglot= once
per project.
