...
Add a hook for restoring =gc-cons-threshold= (set in =early-init.el=) to its
standard value.
...
FIXME: Move =x8dcc/compiled-with-ui= function to helpers section, without moving
the entire section up.
...
Jump between matching keywords and tags with =%=, similar to Vim's built-in
=matchit= plugin. This extends the default =%= behavior (which only matches
single characters like parentheses) to language constructs such as
=if=-=endif=, =begin=-=end=, or HTML tags.
...
We enable the global =magit-wip-mode= to allow tracking of uncommited/unstaged
changes. By using =magit-wip-log=, the user can see and navigate a log of all
uncommited changes (e.g. unstages, restorations, etc.).
...
The original =org-download-clipboard= function also creates an ID property by
calling =org-id-get-create=, which I don't want. By using an advice, we can
temporarily overwrite this =org-id-get-create= function when
=org-download-clipboard= is called.
...
Simple interactive wrapper for =org-download-clipboard=.
...
Auto-closing of brackets and quotes, replacing the built-in
=electric-pair-mode=. Smartparens keeps its own pairing state instead of relying
on the global syntax parser, so typing a closing quote skips over the existing
one reliably, instead of sometimes inserting a duplicate.

The =smartparens-config= library provides sane per-language default pairs.
...
If you get the following error when opening a PDF file:

#+begin_quote
The =pdf-util-convert-program= is unset or non-executable.
#+end_quote

You might need to install =imagemagick= on your system.

*** Ripgrep

Emacs integration with =ripgrep=. I personally don't care about using =grep= or
=ripgrep= in the command line, but the Emacs interface from this package is much
cleaner in my opinion, and is a good replacement for Emacs' =rgrep=.
...
buffer. This is only useful if you haven't enabled vterm's [[https://github.com/akermu/emacs-libvterm/blob/master/README.md#shell-side-configuration][shell-side
configuration]] and [[https://github.com/akermu/emacs-libvterm/blob/master/README.md#directory-tracking-and-prompt-tracking][directory tracking]].
...
Major mode for playing chess.

TODO: Add an /advice/ to =chess-ics= that reads usernames matching the specified
server, asks the user for one (if there are multiple) and passes that to the
original =chess-ics=.
...
First, configure Polymode, the framework for Multiple Major Modes inside a
single buffer.
...
Major mode for [[https://datasheets.raspberrypi.com/pico/raspberry-pi-pico-c-sdk.pdf#instructionSet][PIO assembly files]].
...
Major mode for editing [[https://lilypond.org][LilyPond]] source files.
...
Evaluate a symbol or function to the actual function.
...
Simple wrapper for displaying a buffer in a side window while also selecting it.
...
Convert a title text to a dashed format suitable for filenames or URLs, and
viceversa.
...
Return the standard (original) value of a symbol. See the definition of
=describe-variable= for more information (search for "Original value was").
...
Wrappers for text emphasis in org-mode, bound to =SPC m f ...=.
...
Simple interactive wrapper for toggling window dedication. See [[https://www.masteringemacs.org/article/demystifying-emacs-window-manager][this article]] for
more information on dedicated windows.
...
This function will be used to create a terminal buffer with a /popup/
suffix. These suffixes will be used to create different display rules for base
and popup terminal buffers (see [[*Popper][Popper]]).

Also add wrappers for opening terminals in sidebars.
...
Simple function for resetting the =serial-term= session (e.g. after
disconnecting and reconnecting the serial device).
...
Scroll 3 lines at a time with mouse wheel, and scroll horizontally with shift.
...
Enable precise pixel scrolling, which is specially useful for high-resolution
scroll wheels (which send =REL_WHEEL_HI_RES= events), as opposed to regular ones
(which send =REL_WHEEL=).

1. We also enable interpolation on PageUp and PageDown keys, to make them
   smoother.
2. We set the large scroll height to 1 pixel to ensure all scrolls are
   interpolated.
...
The pixel scroll mode ignores the =mouse-wheel-scroll-amount=, so we need to
redefine =pixel-scroll-precision= to ensure that normal scroll wheels (which send
pixel scroll events greater than N lines) keep the normal "N line scroll"
behavior, along with the pixel smoothness.
...
If the user tries to switch the buffer on strongly dedicated windows, prompt for
confirmation. See [[https://www.masteringemacs.org/article/demystifying-emacs-window-manager][this article]] for more information on dedicated windows.

#+begin_src emacs-lisp
(setq switch-to-buffer-in-dedicated-window 'prompt)
#+end_src

Respect rules in =display-buffer-alist= when the user manually switches buffers.
...
Specify the list of buffers that should be displayed as sidebars on the left.
...
Use a trash directory, instead of deleting files, whenever possible. Note that
=trash-directory= could be customized, or =system-move-file-to-trash= could be
defined; since they aren't, it will use the /freedesktop/ trash, usually
...
Increase the maximum output read from a process. Increases performance of
packages that interact with processes, LSPs, etc.
...
Only support Git as the VC backend.
...
Change the =ls= flags used by Dired. We also need to update the sorting regexps,
so the Dired modeline is correctly set in Emacs 29.3 (see
...
Small function for copying a file to the current directory, overwriting
=dired-dwim-target=.
...
Settings for EWW (Emacs Web Wowser) and SHR (Simple HTML Renderer).

Change the maximum line width to make lines wrap sooner.
...
Org agenda location. First, we make sure that the org directory exists. Then,
for each of the relative paths to agenda files, we concatenate them to the
=org-directory=, ensuring the file exists.
...
Open file links in the same window. This is done by modifying the function
associated to the ='file= key in =org-link-frame-setup=.
...
By default, when exporting tables to HTML, include all borders.

NOTE: The call to =setq= shouldn't be necessary, but the documentation of
=plist-put= recommends it.
...
Default CSS for exported HTML files.
...
Don't use a default width when exporting images to LaTeX (and PDF). The default
value made it so images were 90% of the text width, which didn't make sense if
the image was smaller than that.

#+begin_src emacs-lisp
(setq org-latex-image-default-width "")
#+end_src

To set a "max width" for SVG images, we need to:

1. Include the =adjustbox= package, with the =export= option.
2. Define our own function, which receives a link to the image; if the link
   points to an SVG image, wrap it in ~\adjustbox{max width=\linewidth}{\includesvg{%s}}~.
3. Add an advice for =org-latex--inline-image=, so that our function is called
   before it. It needs =:around= so our function receives the original function as
   the first argument (see the documentation of =advice-add=).
...
Simple wrapper for opening an org link in another window.
...
We will need a function for checking if there is an element with a specific
=CUSTOM_ID= property.
...
Next, our =org-custom-id-get= function. It uses =x8dcc/dashify-string= along with
=x8dcc/org-custom-id-make-unique=, instead of =org-id-new=. I also removed the call
to =org-id-add-location=, since I don't use =org-id-locations=.
...
Respect usual tab width in VHDL mode.
