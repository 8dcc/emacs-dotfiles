...
TODO: Rather than suffixing the number of buffers that match =name=, we should
suffix the next available number. For example, user opens 3 buffers, then closes
the second one; this function should return =name [1]= instead =name [2]= again.

TODO: Also count buffer names ending with =[N]=. Don't change
=x8dcc/count-matching-buffers=.
...
That is, according to `x8dcc/count-matching-buffers'. The suffix is a number
wrapped in square brackets. If there are no buffers with that name, NAME is
returned.
...
Try to reuse the same window for =*grep*= buffers. If it's not possible, display
it in an existing window or in a side window.
