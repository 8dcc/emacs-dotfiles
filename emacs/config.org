...
TODO: Why do we use =mapcar=? Couldn't we just use =dolist=?
...
Alternative to =kill-buffer-and-window= that uses =evil-window-delete=.
...
the indentation of the first line as reference.
...
TODO: Move section to [[*Custom functions and macros][Custom functions and macros]].
