...
(which send =REL_WHEEL=). We also enable interpolation on PageUp and PageDown
keys, to make them smoother.
...
redefine =pixel-scroll-precision= to ensure that normal scroll wheels (which are
detected because they send pixel scroll events greater than a threshold) keep
the normal "N line scroll" behavior, along with the pixel smoothness.
