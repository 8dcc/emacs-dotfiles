...
1. We have to set =evil-want-integration= and =evil-want-keybinding= *before* loading
   both =evil= and =evil-collection= packages.
2. Configure split direction so new splits are opened to the bottom and to the
   right.
3. We set the undo system to a simple =undo-redo=.
4. We set =evil-want-C-i-jump= to =nil= to avoid some =TAB= problems.
5. We set =evil-mode-line-format= for displaying the evil mode before the buffer
   name. See also [[*Modeline][Modeline]].
6. We will use evil's search module because some I-search options don't work
   well with evil (highlight persistence, etc.).
7. We alias =forward-evil-word= to =forward-evil-symbol= and set
