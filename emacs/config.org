...
Disable commit keyword highlighting (in square brackets).
...
=message-send-mail-hook=. Keep in mind that there is a distinction between the
address that is specified in the "From" header, and the credentials used to
authenticate with the SMTP server. We change the former through =user-full-name=
and =user-mail-address=, and the later through =smtpmail-smtp-user=.

FIXME: The authentication with the SMTP server only changes if the value of
=smtpmail-smtp-user= is set globally, not locally in the =message-mode= buffer.
