...
FIXME: The =default-directory= variable contains the path where the =vterm= buffer
was opened, not the current path inside the shell. The shell path should be
saved, but I don't know a way of obtaining this value.
...
terminal buffers (on =display-buffer-alist=, through
=x8dcc/set-display-bottom-window=).
